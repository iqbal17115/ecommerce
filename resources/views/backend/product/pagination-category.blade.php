<table class="table table-striped">
    ...
    <tbody>
        ...
        <tr>
            <th scope="row">{{ ++$i }}</th>
            <td class="category-hierarchy" data-toggle="modal" data-target="#exampleModal" data-id="{{$category->id}}">
                {{$category->name}}</td>
            <td>
                @if($category->Parent)
            ...
            <td>
                @if($category->icon)
                <img src="{{ asset('storage/'.$category->icon) }}" class="rounded" style="width: 55px; height: 40px;" />
                @endif
            </td>
            <td>
                @if($category->image)
            ...
            <td>
                <button type="button" class="btn btn-info text-light btn-sm update_form" data-toggle="modal"
                    ...
                    data-header_menu_position="{{$category->header_menu_position}}" data-icon1="{{$category->icon}}"
                    data-image="{{$category->icon}}" data-icon1="{{$category->image}}" data-image="{{$category->image}}"
